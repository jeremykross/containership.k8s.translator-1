...
const { flatten, unflatten: unflatten } = require('flat');
...
defineMapping(['address', 'private'], [['metadata', 'labels', 'alternative-ip']]);
defineMapping(['cpus'],               [['status', 'capacity', 'cpu']]);
defineMapping(['memory'],             [['status', 'capacity', 'memory']]);
defineMapping(['tags'],               [['metadata', 'labels']]);
defineMapping(['tags', 'host_name'],  [['metadata', 'name']]);
defineMapping(['tags', 'host'],       [['metadata', 'labels', 'cs-node-id']]);
defineMapping(['tags', 'metadata', 'nodeInfo'], [['status', 'nodeInfo']]);
...
defineConversion(['tags'], (v) => {
     const tags = {};

     _.forEach(flatten({ tags: v }), (value, key) => {
...
function csApplicationToK8SReplicationController(csAppDesc) {
    _.defaultsDeep(csAppDesc, constants.application.SCHEMA);
